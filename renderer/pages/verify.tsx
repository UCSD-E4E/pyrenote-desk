...
export default function VerifyPage() {	
  ...
  const [modalEnable, setModalEnable] = useState(false);
      ...
      <div className={styles.modalParent}>
        <section className={styles.selectPopup}>
          ...
          <button onClick={ async () => {
            ...
            handleFileSelectionFromDB(result.recordings);

          }}>Import Selected</button>
          <button onClick={async () => {
            toggleRecordingSelect();
            const recordings = await window.api.listRecordings();
            handleFileSelectionFromDB(recordings);
          }}>Import All</button>
	...
	// initial DB load
	const handleFileSelectionFromDB = async (recordings) => {
					...
					{audioFiles.length > 0 && (
						currentFiles.length > 0 ? (
							<div id="grid" key={forceReloadKey} className={styles.grid} style={{
								...
								gridTemplateRows: `repeat(${ROWS},F auto)`,
							}}>
