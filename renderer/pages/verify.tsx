...
export default function VerifyPage() {	
	...
	const [[audioFiles, setAudioFiles], updateAudioFile] = [useState<ProcessedAudioFile[]>([]), (i, field, v) => {
		setAudioFiles(prevItems => {
			const newItems = [...prevItems]; // make a copy
			newItems[i][field] = v;      // update the element
	...
	async function handleFileSelectionNew() {
		...
		async function handleAudioFile(file, isFlagged=false, species="Default") {
			...
			const tasks = files.map(async (file, i) => {	
				if (file.extension == ".json") { // json file
					...
					for (let j = 0; j < jsonData.table.length; j++) {
						...
						await handleAudioFile(audioFile, entry.isFlagged, entry.species || "Default");
					}
				} else {
				...
				{audioFiles.length > 0 && (
					<>
						<div id="grid" key={forceReloadKey} className={styles.grid} style={{
							...
							{currentFiles.map(({index, filePath, url, isFlagged, species}, i) => {
								return (
									<Spectrogram 
										key={i}
										id={i} 
					...
					<>
						{createPortal(
							<ModalSpectrogram
								...
								species={spectrograms.current[selected].species}
								onMouseEnter={()=>{}}
								onMouseLeave={()=>{}}
								linkedSpectro={spectrograms.current[selected]}
								ref={(el) => {
